...
public:
    ...
    Type *remapType(Type *SrcTy)
    {
        ...
        if (DstTy != SrcTy)
            LLVM_DEBUG(
                    dbgs() << "Remapping type " << SrcTy << " - " << Entry
                           << ":\n"
                           << "  from " << *SrcTy << "\n"
                           << "  to   " << *DstTy << "\n");
        return *Entry = DstTy;
...
struct RemoveAddrspaces : public ModulePass {
    static char ID;
    AddrspaceRemapFunction ASRemapper;
    RemoveAddrspaces(AddrspaceRemapFunction ASRemapper = removeAllAddrspaces)
...
bool RemoveAddrspaces::runOnModule(Module &M)
{
...
char RemoveAddrspaces::ID = 0;
static RegisterPass<RemoveAddrspaces>
        X("RemoveAddrspaces",
...
Pass *
createRemoveAddrspaces(AddrspaceRemapFunction ASRemapper = removeAllAddrspaces)
{
    return new RemoveAddrspaces(ASRemapper);
}
...
Pass *createRemoveJuliaAddrspaces()
{
    return createRemoveAddrspaces(removeJuliaAddrspaces);
}

extern "C" JL_DLLEXPORT void LLVMExtraAddRemoveJuliaAddrspacesPass(LLVMPassManagerRef PM)
{
    unwrap(PM)->add(createRemoveJuliaAddrspaces());
}
